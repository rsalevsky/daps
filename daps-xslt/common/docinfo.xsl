...
<xsl:template name="content">
  ...
  <xsl:call-template name="boilerplate">
    ...
    <xsl:with-param name="value" select="($docinfo/title|$docinfo/db5:title|$docroot/title|$docroot/db5:title)[1]"/>
  </xsl:call-template>
  <xsl:call-template name="boilerplate">
    <xsl:with-param name="attribute" select="'Subtitle'"/>
    <xsl:with-param name="value" select="($docinfo/subtitle|$docinfo/db5:subtitle|$docroot/subtitle|$docroot/db5:subtitle)[1]"/>
  </xsl:call-template>
  <xsl:call-template name="boilerplate">
    <xsl:with-param name="attribute" select="'Product Name'"/>
    <!-- This is inheritable, hence use of $baseinfo. -->
    <xsl:with-param name="value" select="($docinfo/productname|$docinfo/db5:productname|$baseinfo/productname|$baseinfo/db5:productname)[1]"/>
  </xsl:call-template>
  <xsl:call-template name="boilerplate">
    <xsl:with-param name="attribute" select="'Product Version'"/>
    <!-- This is inheritable, hence use of $baseinfo. -->
    <xsl:with-param name="value" select="($docinfo/productnumber|$docinfo/db5:productnumber|$baseinfo/productnumber|$baseinfo/db5:productnumber)[1]"/>
  </xsl:call-template>
  <xsl:call-template name="boilerplate">
